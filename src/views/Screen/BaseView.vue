<template>
    <div class="base-view" :class="{ 'laser-pen': laserPen }">
      <div
        class="slide-click-area"
        @click="handleSlideAreaClick"
      >
        <ScreenSlideList
...
const props = defineProps<{
  ...
  /** 播放页（打开即放映）使用：退出原生全屏时不结束放映 */
  keepPlayingOnFullscreenEsc?: boolean
  /** 播放页使用：点击幻灯片区域翻到下一页（与 Office 放映一致） */
...
const { fullscreenState, manualExitFullscreen } = useFullscreen({ exitScreeningOnEsc: !props.keepPlayingOnFullscreenEsc })

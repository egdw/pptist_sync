  ...
  <div class="studio">
    ...
    <main v-if="module === 'slides'" class="slides-workspace">
      ...
      <section class="preview">
        ...
        <p v-if="draftIsHtml" class="html-markdown-note">此处仍编辑原 Markdown；HTML 的内容和照片请修改原文件后在“页面主题”重新上传。</p>
      ...
      <section class="editor">
        ...
        <template v-if="mode === 'visual' && current">
          ...
          <label>页面排版（决定页面布局，选择后可继续改内容）<select v-model="form.template" @change="applyForm"><option v-if="form.template && !templates.some(t => t.id === form.template)" :value="form.template">{{ form.template }}（当前页面排版）</option><option v-for="t in templates" :key="t.id" :value="t.id">{{ t.name }} · {{ t.desc }}</option></select></label>
    ...
    <main v-else-if="module === 'lcd'" class="lcd-workspace">
      ...
      <section class="lcd-style-panel"><div class="panel-title"><b>LCD Draft 样式</b><span>独立 Renderer</span></div><div class="panel-body"><label>主题<select :value="lcdThemes.find((x:any)=>x.draft)?.id" @change="selectLcdTheme(($event.target as HTMLSelectElement).value)"><option v-for="item in lcdThemes" :key="item.id" :value="item.id">{{item.name}}{{item.active?'（正式）':''}}</option></select></label><label>背景色<input v-model="lcdConfig.background" type="color"></label><label>任务字号<div class="range"><input v-model.number="lcdConfig.taskFontSize" type="range" min="24" max="86"><output>{{lcdConfig.taskFontSize}}</output></div></label><label>岗位字号<div class="range"><input v-model.number="lcdConfig.roleFontSize" type="range" min="28" max="82"><output>{{lcdConfig.roleFontSize}}</output></div></label><label>阶段字号<div class="range"><input v-model.number="lcdConfig.stageFontSize" type="range" min="28" max="82"><output>{{lcdConfig.stageFontSize}}</output></div></label><label>“当前环节”字号<div class="range"><input v-model.number="lcdConfig.stageLabelFontSize" type="range" min="18" max="40"><output>{{lcdConfig.stageLabelFontSize}}</output></div></label><label>岗位编号字号<div class="range"><input v-model.number="lcdConfig.roleIndexFontSize" type="range" min="20" max="48"><output>{{lcdConfig.roleIndexFontSize}}</output></div></label><label>“当前任务”字号<div class="range"><input v-model.number="lcdConfig.taskLabelFontSize" type="range" min="18" max="40"><output>{{lcdConfig.taskLabelFontSize}}</output></div></label><label>负责人/协助人员字号<div class="range"><input v-model.number="lcdConfig.badgeFontSize" type="range" min="18" max="36"><output>{{lcdConfig.badgeFontSize}}</output></div></label><label>底部工作状态字号<div class="range"><input v-model.number="lcdConfig.footerFontSize" type="range" min="18" max="34"><output>{{lcdConfig.footerFontSize}}</output></div></label><label>任务最大行数<input v-model.number="lcdConfig.maxTaskLines" type="number" min="1" max="4"></label><label>头像大小<div class="range"><input v-model.number="lcdConfig.portraitScale" type="range" min="0.35" max="1" step="0.05"><output>{{lcdConfig.portraitScale}}</output></div></label><details class="lcd-css-editor"><summary>CSS 变量微调</summary><p>可覆盖上面五项辅助文字字号；仅解析这些变量，数值会自动限制在安全范围内。</p><textarea v-model="lcdConfig.customCss" spellcheck="false" placeholder=":root {&#10;  --lcd-stage-label-font-size: 28px;&#10;  --lcd-role-index-font-size: 32px;&#10;  --lcd-task-label-font-size: 27px;&#10;  --lcd-badge-font-size: 29px;&#10;  --lcd-footer-font-size: 25px;&#10;}"></textarea></details><button class="wide" @click="saveLcdConfig">保存 LCD Draft</button><div class="divider"></div><div class="avatar-title lcd">LCD 头像 <small>四块 LCD 屏的人头显示，支持 GIF 动图，更换后自动重新渲染</small></div><div class="lcd-avatar-grid"><div v-for="p in lcdPortraits" :key="p.role" class="avatar-slot"><img v-if="p.hasImage" :src="`/led-render-api/portrait/${p.role}?v=${previewBust}`" :alt="roleNames[p.role as Role]" /><div v-else class="avatar-empty">未设置</div><b>{{ roleNames[p.role as Role] }}</b><label class="avatar-upload">更换<input type="file" accept=".png,.jpg,.jpeg,.gif,.webp" hidden @change="e => uploadLcdPortrait(p.role as Role, e)" /></label></div></div><div class="divider"></div><div class="lcd-theme-tools"><a href="/api/studio/lcd/themes/current/download?scope=active">下载正式主题包</a><a href="/api/studio/lcd/themes/current/download?scope=draft">下载 Draft 主题包</a></div><label class="upload-box">上传 LCD Theme ZIP（含 lcd-theme.json）<input type="file" accept=".zip" @change="uploadLcdTheme"></label><details class="lcd-protocol"><summary>主题包协议</summary><pre>{{ lcdProtocol }}</pre></details><details class="lcd-json-editor" @toggle="(e:any)=>{if((e.target as HTMLDetailsElement).open)loadLcdConfigJson()}"><summary>高级编辑（JSON）</summary><textarea v-model="lcdConfigJson" spellcheck="false"></textarea><div class="lcd-json-actions"><button @click="loadLcdConfigJson">重载当前配置</button><button class="primary" @click="saveLcdConfigJson">保存为 Draft</button></div></details><article v-for="item in lcdThemes.filter((x:any)=>x.id!=='default')" :key="item.id" class="theme-row"><span><b>{{item.name}}</b><small>{{item.active?'正式':item.draft?'Draft':''}}</small></span><button v-if="!item.active&&!item.draft" class="danger" @click="deleteLcdTheme(item.id)">删除</button></article></div></section>
    </main>
    ...
    <main v-else class="theme-page">
      <section class="theme-list">
        <h2>页面主题 · CSS / HTML</h2>
        <p>在同一入口上传 CSS 主题 ZIP 或完整 HTML。上传只加入主题库；“设为 Draft”后预览，顶部发布后用于流程副屏，主屏 PPT 不变。</p>
        <div class="download-actions"><a href="/api/studio/themes/current/download?scope=active">下载当前正式文件</a><a href="/api/studio/themes/current/download?scope=draft">下载 Draft 文件</a></div>
        <input type="file" accept=".zip,.html,.htm" :disabled="themeUploading" @change="uploadTheme" />
        <p class="theme-hint">{{themeUploading ? '正在上传与识别页面…' : 'CSS ZIP ≤20MB；自包含 HTML ≤32MB。已支持智证先锋最终版的 16 页自动识别。'}}</p>
        <article v-for="theme in themes" :key="theme.id" :class="{chosen:theme.draft}">
          <span><b>{{ theme.name }}</b><small>{{theme.kind === 'html' ? `HTML · ${theme.pageCount} 个画面` : 'CSS 主题'}}</small><small>{{ theme.active ? '当前正式' : theme.draft ? 'Draft 预览' : '' }}</small><small v-if="theme.warning">{{theme.warning}}</small></span>
          <div><button :disabled="saving || themeUploading" @click="selectTheme(theme.id)">设为 Draft</button><button v-if="theme.id !== 'default' && !theme.active && !theme.draft" class="danger" @click="deleteTheme(theme.id)">删除</button></div>
        </article>
      </section>
      <section class="theme-preview"><h3>{{draftIsHtml ? 'HTML Draft 预览 · 原流程副屏' : 'Draft Theme 预览'}}</h3>
        <iframe :key="previewKey" :src="previewUrl" allow="fullscreen" allowfullscreen></iframe>
        <p v-if="draftIsHtml" class="html-theme-note">HTML 自带布局、样式与交互。这里的预览不会注册副屏、不会触发主屏或 LCD；发布后用原 /reveal/ 副屏。回到 ShowFlow 重载清单并核对稳定页面 ID，不按新旧页号自动配对。</p>
        <details v-else><summary>CSS 高级编辑</summary><textarea v-model="themeCss" spellcheck="false"></textarea><button @click="saveThemeCss">保存 CSS Draft</button></details>
      </section>
    </main>
...
const lcdConfig = reactive({background:'#101b31',taskFontSize:46,roleFontSize:60,stageFontSize:56,stageLabelFontSize:28,roleIndexFontSize:32,taskLabelFontSize:27,badgeFontSize:29,footerFontSize:25,maxTaskLines:3,portraitScale:0.8,customCss:''})
...
const draftIsHtml = computed(() => themes.value.find(t => t.id === (status.value?.draftRevealTheme || status.value?.activeRevealTheme))?.kind === 'html')
const previewUrl = computed(() => {
  const theme = status.value?.draftRevealTheme || status.value?.activeRevealTheme || 'default'
  const editingMarkdown = module.value === 'slides'
  return `/reveal/?studio=draft&studioPage=${editingMarkdown ? selected.value : 0}&studioTheme=${encodeURIComponent(editingMarkdown && draftIsHtml.value ? 'default' : theme)}${editingMarkdown ? '&content=markdown' : ''}`
})
...
function applyForm() {
  ...
  // 排版为空时保留页面原有 class，绝不强制改写（避免 team 等排版被悄悄替换成 action）
  const layout = (form.template || old['class'] || '').trim()
  const managed = new Set(['page-id','stage','lead','active','collab','cue','class']); const preserved = Object.entries(old).filter(([key]) => !managed.has(key)).map(([key,value]) => `data-${key}="${esc(value)}"`)
  const meta = [`class="${esc(layout)}"`, ...preserved, `data-page-id="${esc(id)}"`, `data-stage="${esc(form.stage)}"`, `data-lead="${esc(form.lead)}"`, `data-active="${esc(form.active.join(','))}"`, `data-collab="${esc(collab)}"`, form.cue ? `data-cue="${esc(form.cue)}"` : ''].filter(Boolean).join(' ')
...
async function saveNow() { clearTimeout(timer); saving.value = true; try { const r = await fetch('/api/studio/slides', { method: 'PUT', headers: {'Content-Type':'application/json'}, body: JSON.stringify({ markdown: markdown.value }) }); const data = await r.json(); if (!r.ok) throw new Error(data.error); status.value = data.status; saveLabel.value = '已保存为草稿'; previewKey.value++; return true } catch (e: any) { saveLabel.value = `保存失败：${e.message}`; return false } finally { saving.value = false } }
async function publish() {
  if (!confirm('确认发布当前 Draft？只更新流程副屏，不替换主屏 PPT。正在播放的副屏保持当前画面，新版可手动载入；首次切换 HTML 后请核对编排节点。')) return
  if (module.value === 'slides' && !(await saveNow())) return
  saving.value = true
  try {
    const r = await fetch('/api/studio/publish', {method:'POST',headers:{'Content-Type':'application/json'},body:'{}'})
    const data = await r.json(); if (!r.ok) throw new Error(data.error || '发布失败')
    status.value = data.status; saveLabel.value = '已发布'; await loadVersions(); await loadThemes()
    message.success('发布成功；副屏使用新版前请重载清单并核对编排')
  } catch (error:any) { message.error(error.message || '发布失败，正式版本未切换') }
...
function go(id: string) { history.pushState({}, '', `/studio/${id}`); module.value = id; if (id === 'assets') loadAssets(); if (id === 'theme' || id === 'slides') loadThemes(); if(id==='lcd')loadLcdThemes(); if (id === 'system') loadSystem() }
...
async function loadThemes() {
  const r = await fetch('/api/studio/themes'); if (!r.ok) throw new Error('读取主题列表失败')
  themes.value = (await r.json()).themes
  const css = await fetch('/api/studio/themes/draft/css'); if (css.ok) themeCss.value = (await css.json()).css || ''
}
async function uploadTheme(event:Event) {
  const input=event.target as HTMLInputElement,file=input.files?.[0]; if(!file)return
  if (!/\.(zip|html|htm)$/i.test(file.name)) { message.error('请选择 CSS ZIP 或 HTML 文件'); input.value=''; return }
  const max = /\.zip$/i.test(file.name) ? 20 : 32
  if (file.size > max * 1024 * 1024) { message.error(`文件不能超过 ${max}MB`); input.value=''; return }
  themeUploading.value = true
  try {
    const r=await fetch('/api/studio/themes/upload',{method:'POST',headers:{'X-Filename':encodeURIComponent(file.name),'Content-Type':'application/octet-stream'},body:file})
    const data=await r.json(); if(!r.ok)throw new Error(data.error || '上传失败')
    await loadThemes()
    message.success(data.theme.kind === 'html' ? `已加入 HTML：识别 ${data.theme.pageCount} 个画面，请设为 Draft 预览` : 'CSS 主题已上传，请设为 Draft 预览')
  } catch (error:any) { message.error(error.message || '上传失败') }
  finally { themeUploading.value = false; input.value='' }
}
async function selectTheme(id:string) {
  try { const r=await fetch(`/api/studio/themes/${encodeURIComponent(id)}/preview`,{method:'POST'}); const data=await r.json(); if(!r.ok)throw new Error(data.error);status.value=data.status;previewKey.value++;await loadThemes() }
  catch(error:any){message.error(error.message || '设置 Draft 失败')}
}
async function deleteTheme(id:string) {
  if(!confirm(`确认删除主题 ${id}？`))return
  try { const r=await fetch(`/api/studio/themes/${encodeURIComponent(id)}`,{method:'DELETE'});if(!r.ok)throw new Error((await r.json()).error);await loadThemes() }
...
const lcdBust = ref(Date.now())
async function loadLcdPortraits() {
  const roles = ['manager', 'platform', 'twin', 'hardware']
  lcdPortraits.value = await Promise.all(roles.map(async role => {
    const r = await fetch(`/led-render-api/portrait/${role}`)
    return { role, hasImage: r.ok }
  }))
  lcdBust.value = Date.now()
}
async function uploadLcdPortrait(role: Role, event: Event) {
  const input = event.target as HTMLInputElement; const file = input.files?.[0]
  if (!file) return
  try {
    const r = await fetch(`/led-render-api/portrait/${role}`, { method: 'POST', headers: { 'Content-Type': file.type || 'application/octet-stream', 'X-Filename': encodeURIComponent(file.name) }, body: file })
    const data = await r.json().catch(() => null)
    if (!r.ok || !data?.ok) throw new Error(data?.error || `上传失败（${r.status}）`)
    message.success(`${roleNames[role as Role]} LCD 头像已更新，正在重新渲染…`)
    await loadLcdPortraits()
    await renderStudioLcd()
    lcdMessage.value = 'LCD 头像已更新并重新渲染，/led-preview 将自动切换'
  } catch (e: any) { lcdMessage.value = ''; message.error(e?.message || 'LCD 头像上传失败') }
  input.value = ''
}
...
主题 = lcd-theme.json 配置；可在 customCss 中使用受控 CSS 变量微调。

字段                 范围        默认       说明
background          #RRGGBB    #101b31   背景基色
stageFontSize       28-82       56        顶部环节字号
roleFontSize        28-82       60        岗位名字号
taskFontSize        24-86       46        任务文字字号
stageLabelFontSize  18-40       28        “当前环节”字号
roleIndexFontSize   20-48       32        岗位编号字号
taskLabelFontSize   18-40       27        “当前任务”字号
badgeFontSize       18-36       29        负责人/协助人员徽标字号
footerFontSize      18-34       25        底部工作状态字号
maxTaskLines        1-4         3         任务最大行数
portraitScale       0.35-1      0.8       头像缩放(1=填满人物区)
roleAccents         可选        内置配色   岗位主色覆盖
                     {"manager":"#4f7cff", ...}
inactiveColor       #RRGGBB 可选 #65748d  非活跃岗位文字色
customCss           CSS字符串    空        五项辅助字号的 CSS 变量微调

支持的 CSS 变量：--lcd-stage-label-font-size、--lcd-role-index-font-size、
--lcd-task-label-font-size、--lcd-badge-font-size、--lcd-footer-font-size。
仅支持 px；超出范围会自动收敛，主标题会按安全区域自动换行/缩小。

ZIP 内必须含 lcd-theme.json(仅 JSON,≤20MB);
文件名即主题 ID; 上传→设为 Draft→发布 生效。`
const lcdConfigJson = ref('')
async function loadLcdConfigJson(){lcdConfigJson.value=JSON.stringify(lcdConfig,null,2)}
async function saveLcdConfigJson(){
  try{
    const parsed=JSON.parse(lcdConfigJson.value)
    const id=lcdThemes.value.find((x:any)=>x.draft)?.id||'default'
    const r=await fetch('/api/studio/lcd/themes/draft',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({id,config:parsed})})
    const data=await r.json()
    if(!r.ok)throw new Error(data.error||`保存失败(${r.status})`)
    Object.assign(lcdConfig,data.config);status.value=data.status
    await loadLcdThemes();lcdConfigJson.value=JSON.stringify(data.config,null,2)
    await renderStudioLcd()
    lcdMessage.value='LCD 主题 JSON 已保存为 Draft 并重新渲染'
  }catch(e:any){lcdMessage.value=`JSON 保存失败：${e.message}`}
}
...
async function restore(id:string) { if (!confirm(`将版本 ${id} 复制为 Draft？正式版本不会改变。`)) return; const data = await (await fetch(`/api/studio/versions/${encodeURIComponent(id)}/restore`, {method:'POST'})).json(); status.value = data.status; const slides = await (await fetch('/api/studio/slides')).json(); markdown.value = slides.markdown; selected.value = 0; syncForm(); previewKey.value++; saveLabel.value = '历史版本已复制为草稿'; await loadThemes() }
onMounted(async () => { const data = await (await fetch('/api/studio/slides')).json(); markdown.value = data.markdown; status.value = data.status; syncForm(); await loadVersions(); loadPortraits(); if (module.value === 'assets') loadAssets(); if (module.value === 'theme' || module.value === 'slides') await loadThemes(); if(module.value==='lcd')loadLcdThemes(); if (module.value === 'system') loadSystem() })
</script>
...
.lcd-theme-tools{display:flex;gap:7px;margin:0 0 10px}
.lcd-theme-tools a{flex:1;border:1px solid #cdd4df;border-radius:6px;padding:7px;text-align:center;text-decoration:none;color:#285fc0;background:#f7f9fc;font-size:12px}
.lcd-protocol pre,.lcd-json-editor textarea{width:100%;box-sizing:border-box;font:11px/1.5 Consolas,monospace;white-space:pre-wrap;background:#f7f9fc;border:1px solid #e2e7ef;border-radius:6px;padding:10px;margin:8px 0;color:#40506a}
.lcd-json-editor textarea{min-height:200px;resize:vertical;background:#fff}
.lcd-json-actions{display:flex;gap:8px}
.lcd-css-editor{margin:0 0 14px}.lcd-css-editor summary,.lcd-protocol summary,.lcd-json-editor summary{cursor:pointer;font-size:12px;font-weight:700;color:#40506a}.lcd-css-editor p{font-size:11px;line-height:1.5;color:#7d8797}.lcd-css-editor textarea{width:100%;min-height:150px;resize:vertical;font:11px/1.55 Consolas,monospace;border:1px solid #d5dce7;border-radius:6px;padding:9px}.simple>h2,.theme-list h2{margin-top:0}.simple>p,.theme-list>p{color:#697588;line-height:1.6}.status-grid article{min-height:92px}.assets article{transition:.15s}.assets article:hover{border-color:#9fb2d3;box-shadow:0 4px 14px rgba(35,55,85,.08)}
.avatar-title.lcd{margin:2px 0 10px;font-size:12px;font-weight:700;color:#40506a}
.avatar-title.lcd small{display:block;font-weight:400;color:#98a2b3;margin-top:3px}
.lcd-avatar-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px;margin-bottom:6px}
.lcd-avatar-grid .avatar-slot{display:flex;flex-direction:column;align-items:center;gap:6px;padding:8px;border:1px solid #e2e7ef;border-radius:7px;background:#fff}
.lcd-avatar-grid img,.lcd-avatar-grid .avatar-empty{width:64px;height:76px;object-fit:contain;background:#f2f4f8;border-radius:5px;display:flex;align-items:center;justify-content:center;color:#98a2b3;font-size:10px}
...
.theme-hint,.html-theme-note{color:#58708b;font-size:13px;line-height:1.6;margin:12px 0}.theme-list article span{min-width:0;overflow-wrap:anywhere}.theme-list article small{display:block}.theme-preview{min-width:0}.html-theme-note{padding:12px;background:#edf5ff;border-radius:6px}
.html-markdown-note{margin:0;padding:10px 14px;background:#edf5ff;color:#285580;font-size:13px;line-height:1.7}
</style>

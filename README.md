...
# 📡 Enhancements in This Fork: Presentation Sync + Remote Default PPT (Open & Play)

All upstream features are preserved. This fork adds two self-hosting capabilities for "big-screen presentation / multi-screen sync" scenarios. See [doc/remote-default-ppt.md](/doc/remote-default-ppt.md) (deployment, routes, persistence, verification) and [doc/presentation-link.md](/doc/presentation-link.md) (4-field message protocol) for details. 中文说明见 [README_zh.md](/README_zh.md)。

1. **Presentation Sync (MQTT / WebSocket outbound events)**: On presentation start / slide change / end, a unified 4-field JSON `{event, page, id, notes}` (page number + speaker notes as plain text; the same event shares one id across channels) is pushed via MQTT over WebSocket and/or plain WebSocket channels (independently switchable, with connection status, auto-reconnect and send logs). Settings: editor main menu "≡ → 放映联动", or the top-right button on the play page.
2. **Remote upload & replace the default deck (open & play)**: Ships a zero-dependency Node server. Any computer on the LAN can open `/upload`, upload a .pptx and click "set as default"; the server atomically switches to a new persisted version (raw file + parsed data + metadata), and all open `/play` pages hot-swap to the new deck from slide 1 via an SSE notification. Restarts of the server or browser keep playing the latest default deck.
3. **Routes**: `/` and `/editor` editor (original UI, auto-loads the current default deck); `/play` big-screen play page (opens straight into playback, auto-fullscreen, top-right entry for upload/config/back-to-editor); `/upload` upload manager; `?mode=audience` audience window (upstream).
4. **Run & verify**:

   ```bash
   npm install
   npm run build       # build frontend (with type-check)
   npm run server      # start server (0.0.0.0:8686 by default, page + API + SSE same-origin)
   npm run test:presentation          # presentation-sync targeted tests
   npm run test:default-ppt           # default-PPT server integration tests
   npm run test:default-ppt:browser   # browser end-to-end tests (build first)
   ```

   Docker deployment, data persistence and environment variables (`PPTIST_PORT`, `PPTIST_DATA_DIR`, `PPTIST_PUBLIC_URL`, ...) are documented in [doc/remote-default-ppt.md](/doc/remote-default-ppt.md).

...
export function createStudioService({ rootDir, revealDir, dataDir }) {
  ...
  /** 四岗位头像（reveal 页 markdown 表格与角色卡均按 portraits/{role}.png 引用，
   *  实际文件扩展名可不同，服务端按角色回退解析，GIF 动图原生支持） */
  async function listPortraits() {
    const dir = path.join(revealDir, 'portraits')
    await fsp.mkdir(dir, { recursive: true })
    const names = await fsp.readdir(dir).catch(() => [])
    return Promise.all(PORTRAIT_ROLES.map(async role => {
      const file = names.find(n => n.startsWith(role + '.') && PORTRAIT_EXTENSIONS.includes(path.extname(n).toLowerCase()))
      const stat = file ? await fsp.stat(path.join(dir, file)).catch(() => null) : null
      return {
        role,
        file: file || `${role}.png`,
        exists: !!file,
        url: `/reveal/portraits/${role}.png?v=${stat ? stat.mtimeMs : 0}`,
        updatedAt: stat ? stat.mtime.toISOString() : null,
      }
    }))
  }

  async function savePortrait(role, filename, data) {
    if (!PORTRAIT_ROLES.includes(role)) throw new Error('未知岗位')
    const ext = path.extname(safeName(filename || '')).toLowerCase()
    if (!PORTRAIT_EXTENSIONS.includes(ext)) throw new Error('仅支持 PNG/JPG/JPEG/GIF/WebP（GIF 可为动图）')
    if (!data.length || data.length > 8 * 1024 * 1024) throw new Error('头像不能超过 8MB')
    const dir = path.join(revealDir, 'portraits')
    await fsp.mkdir(dir, { recursive: true })
    // 清理该角色的旧扩展文件，保证一个角色只有一个当前头像
    for (const old of await fsp.readdir(dir).catch(() => [])) {
      if (old.startsWith(role + '.') && PORTRAIT_EXTENSIONS.includes(path.extname(old).toLowerCase()) && old !== role + ext) {
        await fsp.rm(path.join(dir, old), { force: true }).catch(() => {})
      }
    }
    await atomicWrite(path.join(dir, role + ext), data)
  ...
  async function themeInfo(id) {
    ...
    if (await exists(path.join(dir, HTML_META_FILE))) return { ...JSON.parse(await fsp.readFile(path.join(dir, HTML_META_FILE), 'utf8')), id }

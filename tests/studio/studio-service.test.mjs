...
try {
  ...
  const lcdSaved = await service.saveLcdTheme('large-text', { background:'#223344', taskFontSize:72, maxTaskLines:2, stageLabelFontSize:35, customCss:':root{--lcd-footer-font-size: 30px;}' })
  assert.equal(lcdSaved.config.taskFontSize, 72)
  assert.equal(lcdSaved.config.stageLabelFontSize, 35)

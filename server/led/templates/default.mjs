...
export function drawDefaultTemplate(ctx, state, role, portrait) {
  ...
  gradient.addColorStop(0, '#101b31'); gradient.addColorStop(1, '#07101f')
  ...
  ctx.fillStyle = active ? '#fff' : '#aab3c4'; drawWrapped(ctx, state.stage || '—', 76, 150, 700, 2, 56, 64)
  ...
  ctx.fillStyle = active ? '#fff' : '#9ba6b9'; drawWrapped(ctx, cfg.name, 76, 292, 700, 2, 60, 68)
  if (lead) {
  ...
  ctx.fillStyle = active ? '#fff' : '#9ba6b9'; drawWrapped(ctx, state.roles?.[role]?.task || '—', 76, 570, 700, 3, 46, 58)

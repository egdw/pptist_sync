...
# 📡 本仓库新增：放映联动 + 远程默认 PPT（打开即放映）

在保留上游全部功能的基础上，本仓库面向「大屏放映 / 多屏联动」场景新增两组自部署能力，详细说明见 [doc/remote-default-ppt.md](/doc/remote-default-ppt.md)（部署、路径、持久化、验证）与 [doc/presentation-link.md](/doc/presentation-link.md)（四字段消息协议）。

1. **放映联动（MQTT / WebSocket 对外事件）**：放映开始、翻页、结束时，对外推送统一的四字段 JSON `{event, page, id, notes}`（页码 + 演讲者备注纯文本，同一次事件双通道共用同一 id），支持 MQTT over WebSocket 与原生 WebSocket 两条独立通道（可单独/同时启用，含连接状态、自动重连、发送日志）。设置入口：编辑器主菜单「≡ → 放映联动」，播放页右上角也可打开。
2. **远程上传并替换默认 PPT（打开即放映）**：内置零依赖 Node 服务端，局域网内任意电脑打开 `/upload` 上传 .pptx 并「上传并设为默认」，服务端原子切换版本并持久化（原文件 + 解析数据 + 元数据），所有已打开的 `/play` 播放页通过 SSE 收到通知后自动热替换新文稿并从第一页继续放映；服务重启、浏览器重开后仍是最新默认文稿。
3. **页面入口**：`/`、`/editor` 编辑器（原有界面，启动时自动加载当前默认文稿）；`/play` 大屏播放页（打开即放映、自动全屏、右上角可上传/配置/回编辑器）；`/upload` 上传管理页；`?mode=audience` 观众窗口（原有）。
4. **运行与验证**：

   ```bash
   npm install
   npm run build       # 构建前端（含类型检查）
   npm run server      # 启动服务端（默认 0.0.0.0:8686，页面 + API + SSE 同源）
   npm run test:presentation          # 放映联动针对性验证
   npm run test:default-ppt           # 默认 PPT 服务端集成测试
   npm run test:default-ppt:browser   # 浏览器端到端验证（需先 build）
   ```
